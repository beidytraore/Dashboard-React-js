...
function Tablepopulaires(props) {
    ...
    useEffect(() => {
        ...
        async function loadUsers() {
            const response = await axios.get('http://localhost:3000/populars');
        ...
        <>
            ...
            <Fade in={checked} style={{ transitionDelay: checked ? '300ms' : '0ms' }}>
                <div className={classes.root}>
                    <MaterialTable editable={config.editable} options={config.options} localization={config.localization} title="Les livres populaires" columns={config.columns} data={tableData} actions={config.actions}></MaterialTable>
                </div>
            </Fade>
        </>
    );
...
export default Tablepopulaires;