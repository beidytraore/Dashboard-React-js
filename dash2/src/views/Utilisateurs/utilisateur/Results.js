...
function TableUtilisateurs(props) {
    ...
    let config = {
        columns: [
            ...
            { title: 'Date de naissance', field: 'date de naissance'},
        ...
        <>
            ...
            <Fade in={checked} style={{ transitionDelay: checked ? '300ms' : '0ms' }}>
                <div className={classes.root}>
                    <MaterialTable editable={config.editable} options={config.options} localization={config.localization} title="Utilisateur" columns={config.columns} data={tableData} actions={config.actions}></MaterialTable>
                </div>
            </Fade>
        </>
    );
